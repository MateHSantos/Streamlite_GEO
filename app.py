    ...
    if option == 'HomePage':
        ...
        if loja_input:
            ...
            if df_loja.empty:
                ...
                df_loja['<> DELTA'] = df_loja['COLABS_SUGERIDOS'] - df_loja['COLABS_ATIVOS']

                # Calcular % ORC x ATIVO e garantir que não ultrapasse 100%
                df_loja['% ORC x ATIVO'] = (df_loja['COLABS_ATIVOS'] / df_loja['COLABS_SUGERIDOS']) * 100
                df_loja['% ORC x ATIVO'] = df_loja['% ORC x ATIVO'].apply(lambda x: min(x, 100)).round(2)

                # Formatando como porcentagem com 2 casas decimais
                df_loja['% ORC x ATIVO'] = df_loja['% ORC x ATIVO'].astype(str) + '%'
